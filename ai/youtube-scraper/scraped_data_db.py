...
def store_scraped_youtube_data(
    video_url: str,
    ...
    """
    Store YouTube scraped data in the scraped_data table.
    ...
    try:
        ...
        if existing:
            ...
            """, (
                ...
                source_created_at, processing_notes, video_url
            ))
        else:
            # Insert new record
            cur.execute("""
                INSERT INTO scraped_data (
                    ...
                    'youtube', %s, %s, %s, %s, %s, %s, %s, %s, %s, true
                )
            """, (
                video_url, event_type, location, urgency, sentiment, raw_content,
                json.dumps(content_metadata) if content_metadata else None,
        ...
        print(f"Error storing scraped YouTube data: {e}")
        if conn:
            conn.rollback()
        return False
    finally:
...
def get_scraped_youtube_stats() -> Dict[str, int]:
    """Get statistics about scraped YouTube data."""
    conn = None
    try:
        conn = get_connection()
        ...
        print(f"Error getting scraped YouTube stats: {e}")
        return {}
    finally:
        if conn:
            conn.close()

def get_recent_youtube_videos(limit: int = 10) -> List[Dict[str, Any]]:
    """Get recent scraped YouTube videos."""
    conn = None
    try:
        conn = get_connection()
        cur = conn.cursor(cursor_factory=RealDictCursor)
        ...
        print(f"Error getting recent YouTube videos: {e}")
        return []
    finally:
        if conn:
            conn.close()

def get_high_confidence_events(confidence_threshold: int = 80, limit: int = 20) -> List[Dict[str, Any]]:
    """Get high confidence events from scraped data."""
    conn = None
    try:
        conn = get_connection()
        cur = conn.cursor(cursor_factory=RealDictCursor)
        ...
        cur.execute("""
            SELECT * FROM scraped_data 
            WHERE (content_metadata->>'confidence_score')::int >= %s
            ORDER BY scraped_at DESC 
            LIMIT %s
        ...
        print(f"Error getting high confidence events: {e}")
        return []
    finally:
        if conn:
            conn.close()
